...
// SQL Details
const SQLtable = "test1";
const SQLtable2 = "Kalender";
var con = mysql.createConnection({
  host: "localhost",
  user: "KlassenarbeitenWebseite",
  password: "0402PW11!Website",
...
con.connect(function (err) {
  ...
  if (err) {
    ...
    mySQLConnection = "ERROR 500 / ERROR 503";
    kalenderDatenbankConection = "ERROR 500 / ERROR 503";
  }
...
app.post("/mysql/regestrieren", (req, res) => {
  ...
  if (!benutzername || benutzername === "") {
    return (
      res.status(406).json("Benutzername ist Pflicht!"),
  ...
  if (!passwort || passwort === "") {
    return (
      res.status(406).json("Passwort ist Pflicht!"),
  ...
  if (passwort.length < 7) {
    return (
      res.status(406).json("Passwort ist zu kurz!"),
  ...
  if (passwort !== passwortWDH) {
    return (
      res.status(431).json("Passwort ist nicht gleich!"),
  ...
  if (email && email !== "") {
    ...
    if (email.length > 40) {
      return (
        res.status(431).json("Email ist zu lang!"),
  ...
  if (!kuerzel || kuerzel === "") {
    return (
      res.status(406).json("Kürzel ist Pflicht!"),
  ...
  if (!gueltigeKuerzel.includes(kuerzel)) {
    return (
      res.status(406).json("Kürzel ist ungültig!"),
  ...
  con.query(sqlUser, [benutzername], (err, userResult) => {
    ...
    if (userResult.length > 0) {
      return (
        res.status(409).json("Benutzername ist bereits vergeben"),
    ...
    if (emailAngegeben) {
      con.query(sqlEmail, [email], (err, emailResult) => {
        ...
        if (emailResult.length > 0) {
          return (
            res.status(409).json("Email ist bereits vergeben"),
  ...
  if (kalenderDatenbankConection == "Code 200") {
    ...
    con.query(
      ...
      (err, data) => {
        ...
        if (data.length > 0) {
          console.log(
            "Server und Datenbank sind erreichbar, Datenbank und Table Kalender sind nicht down",
          );
          res
            .status(200)
            .json("Code 200, Datenbank und Server sind nicht down");
          return;
        } else {
